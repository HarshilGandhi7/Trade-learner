...
export default function BITCOINT_PAGE() {
  ...
  const fetchHistoricalData = async (selectedTimeframe = timeframe) => {
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_CRYPTO_CHART_URL}/api/v3/coins/bitcoin/market_chart?vs_currency=usd&days=${days}`
      );

      if (!response.ok) {
        throw new Error(`Bitcoin API error! Status: ${response.status}`);
      }
      ...
      if (data.prices.length > 0) {
        ...
        setData((prevData:CryptoData | null) => ({
          ...
          volume24h: data.total_volumes.reduce((acc: number, curr: [number, number]) => acc + curr[1], 0),
          marketCap: data.market_caps.reduce((acc: number, curr: [number, number]) => acc + curr[1], 0),
          lastUpdate: Date.now(),
        }));
      ...
      <div className="max-w-7xl mx-auto py-4 px-4">
        ...
        <div className="mt-8">
          ...
          <div className="mt-3 mb-6 p-4 bg-zinc-800/50 rounded-md border border-zinc-700 text-zinc-300 text-sm">
            ...
            The world's first and largest cryptocurrency — operates on a
            decentralized blockchain network, providing peer-to-peer
            transactions without intermediaries or central authority.
            <button
            ...
            {showMore && (
              <p className="mt-2 text-xs text-zinc-400">
                ...
                Bitcoin has a fixed supply cap of 21 million coins. It uses
                proof-of-work consensus to validate transactions and has become
                widely recognized as "digital gold" — a hedge against inflation
                and currency devaluation during economic uncertainty.
              </p>
            )}
          </div>
          ...
          <CryptoData 
            symbol={process.env.NEXT_PUBLIC_BITCOIN_SYMBOL || "BTCUSDT"}
            name={Name}
