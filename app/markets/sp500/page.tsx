    ...
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get(
          `https://finnhub.io/api/v1/quote?symbol=${process.env.NEXT_PUBLIC_SP500_SYMBOL}&token=${process.env.NEXT_PUBLIC_SP500_API_KEY}`
        );
    ...
    const manageConnection = () => {
      ...
      if (isOpen && !socketRef.current) {
        socketRef.current = new WebSocket(
          `wss://ws.finnhub.io?token=${process.env.NEXT_PUBLIC_SP500_API_KEY}`
        );
        socketRef.current.addEventListener("open", function () {
          if (socketRef.current) {
            socketRef.current.send(
              JSON.stringify({
                type: "subscribe",
                symbol: process.env.NEXT_PUBLIC_SP500_SYMBOL,
              })
            );
          }
        });
